...
class RegisterAPIView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            email_obj = EmailMessage('email/verification_email.tpl', {'token': token}, "admin@admin.com",
                       to=[email])
...
class VerificationAPIView(APIView):
    def get(self, request, token, *args, **kwargs):
        ...
        if user_obj.is_verified:
            return Response({"detail":"your account is verified already"}, status=status.HTTP_400_BAD_REQUEST)    
        user_obj.is_verified = True
        user_obj.save()
        return Response({"detail":"your account verified successfilly"}, status=status.HTTP_200_OK)
...
class ResendVerificationAPIView(generics.GenericAPIView):
    ...
    def post(self, reauest, *args, **kwargs):
        ...
        token = self.get_token_for_users(user_obj)
        email_obj = EmailMessage('email/verification_email.tpl', {'token': token}, "admin@admin.com",to=[user_obj.email])
        EmailThreading(email_obj).start()
        return Response({"detail":"Verification Resend"}, status = status.HTTP_201_CREATED)
